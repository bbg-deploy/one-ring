module AuthenticationSharedExamples
  shared_examples_for "devise attributes (non-employee)" do |factory|
    describe "username" do
      it_behaves_like "attr_accessible", factory, :username,
        ["Chris", "James", "tHomAS", "isexactlytwentychars"],  #Valid values
        [nil, "!", "cat", "Chris Topher", "admin", "demo", "12Street", "ithastoomanycharacters"] #Invalid values
      
      it_behaves_like "unique attr_accessible", factory, :username, "username"

      describe "case-insensitivity" do
        it "downcases username" do
          user = FactoryGirl.create(factory, :username => "DemOUSErnaMe")
          user.username.should eq("demousername")
        end
      end

      describe "whitespace stripping" do
        it "trims whitespace from the front" do
          user = FactoryGirl.create(factory, :username => " whitespace")
          user.username.should eq("whitespace")
        end
        
        it "trims whitespace from the end" do
          user = FactoryGirl.create(factory, :username => "whitespace ")
          user.username.should eq("whitespace")
        end

        it "does not allow whitespace in the middle" do
          expect { FactoryGirl.create(factory, :username => "white space") }.to raise_error
          FactoryGirl.build(factory, :username => "white space").should_not be_valid
        end
      end
    end

    describe "email" do
      it_behaves_like "confirmable attr_accessible", factory, :email,
        ["valid@notcredda.com"], #Valid values
        ["valid@credda.com", "valid.notcredda.com", "@notcredda.com"] #Invalid values

      describe "case-insensitivity" do
        it "downcases email" do
          user = FactoryGirl.create(factory, :email => "TEST@notcredda.com")
          user.email.should eq("test@notcredda.com")
        end
      end

      describe "whitespace stripping" do
        it "strips leading whitespace" do
          user = FactoryGirl.create(factory, :email => " white@notcredda.com")
          user.email.should eq("white@notcredda.com")
        end
    ...
    describe "password" do
      it_behaves_like "confirmable attr_accessible", factory, :password,
        ["S3curePass", "ValidPassW0rd"],  #Valid values
        [nil, "!", "123", "abc"] #Invalid values
    end

    describe "terms agreement" do
      it "requires terms_agreement on create" do
        expect{ user = FactoryGirl.create(factory, :terms_agreement => "1") }.to_not raise_error
      end

      it "fails create with false terms_agreement" do
        expect{ user = FactoryGirl.create(factory, :terms_agreement => "0") }.to raise_error
      end

      it "does not require terms agreement on update" do
        user = FactoryGirl.create(factory)
        user.update_attributes(:username => "changed", :terms_agreement => "0")
        user.username.should eq("changed")
      end
    end
  end

  shared_examples_for "devise attributes (employee)" do |factory|
    describe "username" do
      it_behaves_like "attr_accessible", factory, :username,
        ["James", "tHomAS", "isexactlytwentychars"],  #Valid values
        [nil, "!", "cat", "Chris Topher", "admin", "demo", "12Street", "ithastoomanycharacters"] #Invalid values
      ...
      describe "whitespace stripping", :failing => true do
        it "trims whitespace from the front" do
          user = FactoryGirl.create(factory, :username => " whitespace")
          user.username.should eq("whitespace")
        end
    ...
    describe "email" do
      it_behaves_like "confirmable attr_accessible", factory, :email,
        ["valid@credda.com"], #Valid values
        ["valid@notcredda.com", "valid.credda.com", "@credda.com"] #Invalid values

      describe "case-insensitivity" do
        it "downcases email" do
          user = FactoryGirl.create(factory, :email => "TEST@credda.com")
          user.email.should eq("test@credda.com")
        end
      end

      describe "whitespace stripping" do
        it "strips leading whitespace" do
          user = FactoryGirl.create(factory, :email => " white@credda.com")
          user.email.should eq("white@credda.com")
        end
