class Customer < ActiveRecord::Base
  include ActiveModel::Validations
  require 'fuzzystringmatch'
  include Authentication
  has_friendly_username
  is_authenticatable
  is_registerable
  is_confirmable
  is_recoverable
  is_lockable
  is_rememberable
  is_timeoutable
  is_terms_agreeable

  def devise_mailer
    CustomerAuthenticationMailer
  end

  # Associations - Leases
  #----------------------------------------------------------------------------
  has_many :lease_applications, :inverse_of => :customer
  has_many :leases, :through => :lease_applications

  # Associations - Payments
  #----------------------------------------------------------------------------
  has_many :payment_profiles, :dependent => :destroy, :inverse_of => :customer

  # Associations - Contact Information
  #----------------------------------------------------------------------------
  has_one :mailing_address, :as => :addressable, :class_name => 'Address', 
                            :conditions => {:address_type => :mailing}, :dependent => :destroy, :inverse_of => :addressable
  accepts_nested_attributes_for :mailing_address
  
  has_one :phone_number, :as => :phonable, :class_name => 'PhoneNumber', :dependent => :destroy, :inverse_of => :phonable
  accepts_nested_attributes_for :phone_number

  # Associations - Income
  #----------------------------------------------------------------------------
  has_many :income_sources, :dependent => :destroy, :inverse_of => :customer

  # Accessible Methods
  #----------------------------------------------------------------------------
  attr_accessible :first_name, :middle_name, :last_name, :date_of_birth, :social_security_number,
                  :mailing_address, :mailing_address_attributes, 
                  :phone_number, :phone_number_attributes

  # Validations
  #----------------------------------------------------------------------------
  validates :email, :not_credda_email => true
  validates :first_name, :presence => true, :name_format => true
  validates :middle_name, :name_format => true
  validates :last_name, :presence => true, :name_format => true
  validates :date_of_birth, :presence => true
  validates_date :date_of_birth, :before => lambda { 18.years.ago },
                                 :before_message => "must be at least 18 years old"
  validates :social_security_number, :presence => true,
                                     :uniqueness => true,
                                     :social_security_number_format => true,
                                     :allow_nil => true
  validates :mailing_address, :presence => true
  validates_associated :mailing_address
  validates :phone_number, :presence => true
  validates_associated :phone_number
  validates_associated :payment_profiles
  validates_associated :lease_applications

  # Public Methods
  #----------------------------------------------------------------------------
  public
  # Views Helpers (Probably should go in a helpers file)
  #----------------------------------------------------
  def name
    return "#{self.first_name} #{self.last_name}"
  end
  
  # Controller Methods
  #----------------------------------------------------
  def unclaimed_lease_applications
    jarow = FuzzyStringMatch::JaroWinkler.create(:pure)

    unclaimed = LeaseApplication.unclaimed
    matches = []
    unclaimed.each do |lease|
      if (jarow.getDistance(self.email, lease.matching_email) > 0.90)
        matches << lease
      end
    end

#    matcher = FuzzyMatch.new(Lease.unclaimed_contracts, :read => :matching_email)
#    matches = matcher.find(self.email)
    return matches
  end

  def claimed_lease_applications
    return LeaseApplication.claimed.where(:customer_id => self.id).order('created_at ASC')
  end

  def submitted_lease_applications
    return LeaseApplication.submitted.where(:customer_id => self.id).order('created_at ASC')
  end

  def approved_lease_applications
    return LeaseApplication.approved.where(:customer_id => self.id).order('created_at ASC')
  end

  def denied_lease_applications
    return LeaseApplication.denied.where(:customer_id => self.id).order('created_at ASC')
  end

  def finalized_lease_applications
    return LeaseApplication.finalized.where(:customer_id => self.id).order('created_at ASC')
  end

  def completed_lease_applications
    return LeaseApplication.completed.where(:customer_id => self.id).order('created_at ASC')
  end
  private
end